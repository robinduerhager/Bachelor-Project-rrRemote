...
class AMQPClient implements amqpClientOptions {
  ...
  public sendRPCMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!success) reject(new Error('Unable to publish RPC Call'))
    })
  }
    ...
    this.channel
      .consume(
        ...
        async msg => {
          ...
          const cbResult: object | void = await callback(msg).catch(err => {
            // if an error occurs, the message should be erased from RabbitMQ, so the service does not try to periodically handle this message
            this.acknowledge(msg)
            if (err instanceof RRRemoteError)
            ...
            if (err instanceof Error)
              ...
              logger.error('An Error occured in an RPC callback function', {
                err
              })
          })
  ...
  public async addListener(
    ...
    callback: (msg: amqp.ConsumeMessage) => Promise<any>
  ) {
      ...
      msg => {
        callback(msg).catch(err => {
          ...
          if (err instanceof Error)
            ...
            logger.error('Error occured in AMQP Listener', {
              err
            })
        })
      },
